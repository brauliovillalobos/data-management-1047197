...
FROM olympics_scen_3.games a, olympics_scen_3.games_city b, olympics_scen_3.city c
...
FROM 	(SELECT games_competitor.id, region_name
		FROM olympics_scen_3.games_competitor
		LEFT JOIN olympics_scen_3.person
		ON games_competitor.person_id = person.id
		LEFT JOIN olympics_scen_3.person_region 
		ON person.id = person_region.person_id
		WHERE games_id IN 	(SELECT b.id
							FROM olympics_scen_3.games_competitor AS a, olympics_scen_3.games AS b
							WHERE a.games_id = b.id AND games_year > 1960 AND games_year < 2016)) AS principal
INNER JOIN (SELECT competitor_id, sport_name
			FROM olympics_scen_3.competitor_event AS a, olympics_scen_3.event AS b, olympics_scen_3.sport AS c
			WHERE a.event_id = b.id AND b.sport_id = c.id AND medal_id <> 4 AND sport_id IN (
				...
				FROM 	(SELECT sport_id, COUNT(*) AS total_competitors 
						FROM olympics_scen_3.competitor_event AS a, olympics_scen_3.event AS b    
...
FROM(SELECT id, games_id, person_id
	FROM olympics_scen_3.games_competitor AS a
	INNER JOIN(
		SELECT competitor_id
		FROM olympics_scen_3.competitor_event
		WHERE medal_id <> 4) AS b
	ON a.id = b.competitor_id) AS fir
LEFT JOIN(	SELECT e.id, e.full_name,f.region_name
			FROM olympics_scen_3.person AS e, olympics_scen_3.person_region AS f
			WHERE e.id = f.person_id AND f.region_id NOT IN(SELECT DISTINCT(country_id)
																			FROM olympics_scen_3.city)) AS sec
...
WITH order_values AS(
	...
	FROM(
		...
		FROM olympics_scen_3.games_competitor a
		LEFT JOIN olympics_scen_3.games b
		ON a.games_id = b.id
...
INNER JOIN olympics_scen_3.person AS thi
...
FROM olympics_scen_3.games_competitor AS a 
LEFT JOIN olympics_scen_3.competitor_event AS b
ON a.id = b.competitor_id
WHERE person_id NOT IN(	SELECT DISTINCT(person_id)
						FROM olympics_scen_3.games_competitor a 
						LEFT JOIN olympics_scen_3.competitor_event b
						ON a.id = b.competitor_id
						WHERE medal_id <> 4)
...
FROM perseverants
LEFT JOIN olympics_scen_3.event AS event_table
...
LEFT JOIN (	SELECT eins.id, eins.full_name, eins.gender, eins.height, eins.weight, zwei.region_name
			FROM olympics_scen_3.person AS eins, olympics_scen_3.person_region AS zwei
