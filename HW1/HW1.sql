...
#without ever winning a medal, on the SAME discipline, for the greatest number of olympic games (at least twice).
#*******************************************************************************************************
...
ON a.id = b.competitor_id
LEFT JOIN olympics_changed.event AS c
ON b.event_id = c.id;
...
#Checking person 83312 with discipline 287
#First, check that this person never won a medal
SELECT * 
FROM olympics_changed.games_competitor AS a
LEFT JOIN olympics_changed.competitor_event AS b
ON a.id = b.competitor_id
WHERE person_id = 83312 AND medal_id <>4; #Verified! This person never won a medal!

#Something weird happens with participants of Art and Drawing as they might participate several times in the same Olympic games,
#under the same discipline. This discipline is very old and there might be data inconsistencies with it. However, if we take for instance
#person 93650 in the discipline 279 (Table Tennis Men's Singles) we verify this person participated in 7 different Olympic games,
#in the same discipline, without winning a medal. We might want to consider only more recent data to avoid these possible inconsistencies.

#Checking how many times this person 83312/93650 participated in the discipline 287/279
SELECT * 
FROM olympics_changed.games_competitor AS a
LEFT JOIN olympics_changed.competitor_event AS b
ON a.id = b.competitor_id
LEFT JOIN olympics_changed.event AS c
ON b.event_id = c.id
LEFT JOIN olympics_changed.games AS z
ON games_id = z.id
WHERE person_id = 93650 AND event_id = 279;